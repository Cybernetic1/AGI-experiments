"""
Debugging run for Transformer baseline: per-batch loss and gradient norm logging,
plus best-eval checkpointing. Intended to reproduce seed-dependent divergence.

Usage: python pot-demo/train_pot_transformer_debug.py --data ... --seed 9 --epochs 20 --deterministic
"""
...
def evaluate(model, rows, src_vocab, tgt_vocab, device):
    ...
    with torch.no_grad():
        for row in rows:
            input_ids = torch.tensor(encode_input(row, src_vocab), dtype=torch.long, device=device).unsqueeze(1)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for bidx, idx in enumerate(order):
            row = train_rows[idx]
            src_ids = torch.tensor(encode_input(row, src_vocab), dtype=torch.long, device=device).unsqueeze(1)
            tgt_ids = encode_target(row, tgt_vocab)
            tgt_in = [1] + tgt_ids
            tgt_out = tgt_ids + [2]
            tgt_in_t = torch.tensor(tgt_in, dtype=torch.long, device=device).unsqueeze(1)
            tgt_out_t = torch.tensor(tgt_out, dtype=torch.long, device=device)
            T = tgt_in_t.size(0)
            tgt_mask = nn.Transformer.generate_square_subsequent_mask(T).to(device)
            logits = model(src_ids, tgt_in_t, src_key_padding_mask=None, tgt_key_padding_mask=None, tgt_mask=tgt_mask)
            ...
            if bidx % 10 == 0:
                stats["batches"].append({"epoch": epoch, "batch": bidx, "loss": loss.item(), "grad_norm": grad_norm})
                # print a short live line
                print(f"E{epoch:02d} B{bidx:03d} loss={loss.item():.4f} grad_norm={grad_norm:.4f}", flush=True)
        ...
        if metrics["exact"] > best_exact:
            ...
            best_epoch = epoch
            torch.save({"model_state": model.state_dict(), "optim_state": optim.state_dict(), "epoch": epoch}, f"/tmp/transformer_seed{args.seed}_best.pt")

    print(f"Best epoch: {best_epoch} exact={best_exact:.3f}", flush=True)
    # dump stats to a small json for inspection
    outp = Path(f"/tmp/transformer_seed{args.seed}_debug.json")
    outp.write_text(json.dumps(stats, indent=2))
